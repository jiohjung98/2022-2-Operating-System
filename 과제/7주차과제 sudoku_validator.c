	...
	for (i = 0; i < 9; i++) {
		for (j = 0; j < 9; j++) {						
			...
			if (i%3 == 0 && j%3 == 0) {
				...
				printf("%2dth thread created with \'is3x3Valid\' function at \t[%d][%d]:%d\n", threadIndex, i, j, sudoku[i][j]);
			}
			// THREAD: column check
			if (i == 0) {
				...
				// threads 리스트의 threadIndex위치에 쓰레드를 생성
				printf("%2dth thread created with \'isColumnValid\' function at \t[%d][%d]:%d\n", threadIndex, i, j, sudoku[i][j]);
			}
			// THREAD: row checking
			if (j == 0) {
				...
				// threads 리스트의 threadIndex위치에 쓰레드를 생성
				printf("%2dth thread created with \'isRowValid\' function at \t[%d][%d]:%d\n", threadIndex, i, j, sudoku[i][j]);
			}				
		}
	}
